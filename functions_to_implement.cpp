...
std::vector<std::string> Split(std::string whole, std::string separator) {
	...
	int separator_i = 0;
	int whole_length = whole.length();
	std::vector<std::string> vec;
	std::string current_str = "";
	std::string separator_str = "";

	for (int i = 0; i < whole_length; i++) {
		if (whole[i] == separator[separator_i]) {
			separator_str += whole[i];
			separator_i += 1;
			if (separator_i == separator_length) {
				if (current_str != "") {
					vec.push_back(current_str);
				}
				separator_str = "";
				separator_i = 0;
				current_str = "";
	...
	// Include leftover? I think so.
	if (current_str != "" && current_str != separator) {
		vec.push_back(current_str);
	}
	return vec;
}
...
std::string RemoveFirstSubstring(std::string s1, std::string s2) {
	...
	for (int i = 0; i < s1Length; i++) {
		if (s1[i] == s2[s2i]) {
			...
			s2i += 1;
			if (s2i == s2Length) {
				return s1.erase(s2Start, s2Length);
			}
		} else {
			s2i = 0;
		}
